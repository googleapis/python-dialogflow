...
def test_documents_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.dialogflow_v2.services.documents.transports.DocumentsGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_reload_document_flattened():
    ...
    with mock.patch.object(type(client.transport.reload_document), "__call__") as call:
        ...
        # using the keyword arguments to the method.
        client.reload_document(
            name="name_value", content_uri="content_uri_value",
        )
...
@pytest.mark.parametrize("request_type", [document.ExportDocumentRequest, dict,])
def test_export_document(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.export_document), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name="operations/spam")
...
@pytest.mark.asyncio
async def test_export_document_async(
    transport: str = "grpc_asyncio", request_type=document.ExportDocumentRequest
):
    client = DocumentsAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(), transport=transport,
    )
...
def test_documents_base_transport():
    ...
    with mock.patch(
        "google.cloud.dialogflow_v2.services.documents.transports.DocumentsTransport.__init__"
    ) as Transport:
...
def test_documents_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.cloud.dialogflow_v2.services.documents.transports.DocumentsTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
