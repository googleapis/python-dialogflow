...
def test_get_answer_record_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.AnswerRecordsRestInterceptor, "post_get_answer_record") as post, \
         mock.patch.object(transports.AnswerRecordsRestInterceptor, "pre_get_answer_record") as pre:
        ...
        req.return_value._content = answer_record.AnswerRecord.to_json(answer_record.AnswerRecord())
        ...
        client.get_answer_record(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_get_answer_record_rest_bad_request(transport: str = 'rest', request_type=answer_record.GetAnswerRecordRequest):
    client = AnswerRecordsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_answer_record_rest(request_type):
    ...
    request_init["answer_record"] = {'name': 'projects/sample1/answerRecords/sample2', 'answer_feedback': {'correctness_level': 1, 'agent_assistant_detail_feedback': {'answer_relevance': 1, 'document_correctness': 1, 'document_efficiency': 1, 'summarization_feedback': {'start_timestamp': {'seconds': 751, 'nanos': 543}, 'submit_timestamp': {}, 'summary_text': 'summary_text_value'}}, 'clicked': True, 'click_time': {}, 'displayed': True, 'display_time': {}}, 'agent_assistant_record': {'article_suggestion_answer': {'title': 'title_value', 'uri': 'uri_value', 'snippets': ['snippets_value1', 'snippets_value2'], 'metadata': {}, 'answer_record': 'answer_record_value'}, 'faq_answer': {'answer': 'answer_value', 'confidence': 0.1038, 'question': 'question_value', 'source': 'source_value', 'metadata': {}, 'answer_record': 'answer_record_value'}}}
    request = request_type(**request_init)
...
def test_answer_records_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.dialogflow_v2beta1.services.answer_records.transports.AnswerRecordsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_answer_records_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.dialogflow_v2beta1.services.answer_records.transports.AnswerRecordsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AnswerRecordsTransport()
        adc.assert_called_once()
